...
module.exports = async function(callback) {
	try {
		// Fetch the accounts from wallet - these are unlocked
		...
		//Give tokens toi the account1
		const sender = accounts[0]
		const receiver = accounts[1]
		let amount = web3.utils.toWei('10000', 'ether') // 10000tokens
		...
		console.log(`Filled order ${user1}`)
		...
		console.log(`Made order of 200 tokens for 0.15 ether from ${user1}`)


		//user2 fill the final order
		orderId = result.logs[0].args.id
		await exchange.fillOrder(orderId, { from: user2 })
		console.log(`Filled order from ${user1}`)
